...
class LoggerThread(threading.Thread):
    ...
    def run(self):
        while self.running:
            ...
            # 3) Double barrier wait so the logger doesn't race ahead
            try:
                self.barrier.wait()
            except threading.BrokenBarrierError:
                break
    ...
    def print_system_snapshot(self, t):
        ...
        for i, subsystem in enumerate(self.subsystems, start=1):
            ...
            # If subsystem has a waiting queue, print it
            ...
            # 1) Case: Subsystem1 has multiple ready queues
            ...
            # 2) Case: Subsystem2 / Subsystem3 have a single PriorityQueue
            elif hasattr(subsystem, 'ready_queue') and \
                 self.is_priority_queue(subsystem.ready_queue):
                print(f"  Ready Queue: {self.inspect_priority_queue(subsystem.ready_queue)}")

                # If it has multiple 'cores' (SRJF) or a single 'core' (RateMonotonic)
            ...
            # 3) Case: Subsystem4 (FCFS) has a normal Queue and two cores (core1, core2)
            elif hasattr(subsystem, 'ready_queue') and \
                 not self.is_priority_queue(subsystem.ready_queue) and \
                 hasattr(subsystem, 'core1') and hasattr(subsystem, 'core2'):
                # Print ready queue for normal Queue
                print(f"  Ready Queue: {self.inspect_normal_queue(subsystem.ready_queue)}")

                # Print each core’s running task
                rt1 = self.get_core_running_task_id(subsystem.core1)
                rt2 = self.get_core_running_task_id(subsystem.core2)
                print(f"  Core1: Running Task: {rt1 if rt1 else 'idle'}")
    ...
    def get_core_running_task_id(self, core):
        """For Subsystem2/3/4's single or dual cores."""
        if hasattr(core, 'current_task') and core.current_task is not None:
            return core.current_task.task_id
    ...
    def is_priority_queue(self, q):
        """Check if 'q' is a PriorityQueue instead of a normal Queue."""
        return hasattr(q, 'mutex') and hasattr(q, 'queue') \
            and hasattr(q, 'heap')  # or some other distinguishing check

    def inspect_priority_queue(self, pq):
        """Return a list of task_ids from a priority queue (2-tuple or 3-tuple)."""
        results = []
        with pq.mutex:  # optional (if needed for thread safety)
            for item in pq.queue:
                if len(item) == 2:
                    # (priority, task)
                    results.append(item[1].task_id)
                elif len(item) == 3:
                    # (priority, some_int, task)
                    results.append(item[2].task_id)
        return results

    def inspect_normal_queue(self, q):
        """Return a list of task_ids from a normal Queue() of Task objects."""
        with q.mutex:  # optional, depends on Python version
            return [task.task_id for task in q.queue]
